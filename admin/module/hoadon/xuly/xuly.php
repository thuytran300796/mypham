...
<?php
	...
	if($ac != "")
	{
		...
		else
		{
			...
			$sql = " and date(hd.ngayxuat) >= '$ngaybd' and date(hd.ngayxuat) <= '$ngaykt' ";
		}
	}
	else
		$sql = "";
	...
	$hoadon = mysql_query("
		...
		$cthd = mysql_query("select t1.mahd, t1.mactsp, t1.tensp, t1.mausac, t1.ngaysx, t1.hansudung, t1.soluong, t1.giaban, t2.makm, t2.chietkhau, t2.tiengiamgia, t1.quatang, t1.thue
							from
							(	select	cthd.mahd, cthd.mactsp, tensp, ctsp.mausac, ctsp.ngaysx, ctsp.hansudung, cthd.soluong, ctsp.giaban, cthd.makm, sp.thue, cthd.quatang
								from	chitiethoadon cthd, chitietsanpham ctsp, hoadon hd, sanpham sp
								where	cthd.mactsp = ctsp.mactsp and hd.mahd = cthd.mahd and sp.masp = ctsp.masp".$sql."
							)t1 left join
							(
	...
	/*		
	echo "<table width='100%' class='tb-lietke' >";
    
    echo "<tr>";
    echo 	"<th width='6%'>Mã HĐ</th>";
    echo	 "<th width='9%'>Ngày đặt</th>";
    echo 	"<th width='12%'>Tên KH</th>";
    echo 	"<th width='18%'>Địa chỉ giao hàng</th>";
    echo 	"<th width='8%'>SĐT</th>";
    echo 	"<th width='7%'>Tổng tiền</th>";
    echo 	"<th width='8%'>Trạng thái</th>";
    echo 	"<th width='5%'>Hủy</th>";
    echo "</tr>";
    */
	...
	?>
		...
		?>        
            ...
            <div class='lietke-sp-td' style='width: 4%; text-align: left;'><a href='#'>Hủy</a></div>
            <div class="clear"></div>
        </div>
