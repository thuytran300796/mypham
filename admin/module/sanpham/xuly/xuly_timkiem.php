<?php
	...
	else if($ac == 'search')
	{
		...
		$q_sp = "select sp.masp, sp.tensp, tenncc, sum(ctsp.SoLuong) as 'sl'
				...
				where sp.masp = ctsp.masp and ncc.mancc = sp.mancc and sp.trangthai = 1 and ctsp.TrangThai = 1 and (sp.tensp LIKE '%$keyword%' OR sp.masp LIKE '%$keyword%')".$sql."
				group by sp.masp";
		
		$q_ctsp = "select sp.masp, ctsp.mactsp, mausac, ctsp.ngaysx, ctsp.hansudung, ctsp.soluong, ctsp.giaban
				from sanpham sp, chitietsanpham ctsp
				where sp.masp = ctsp.masp and ctsp.trangthai = 1 and sp.trangthai = 1".$sql."";
...
<?php
	...
	function Load_Loc($q_sp, $q_ctsp)
	{
		...
		while($re_ctsp = mysql_fetch_assoc($ctsp))
		{
			...
			echo	"<div class='lietke-sp-td' style='width: 7%;'>".$re_ctsp['mausac']."</div>";
			...
			echo 	"<div class='lietke-sp-td' style='width: 4%;'>".$re_ctsp['soluong']."</div>";
...
<?php
	...
	function Load_SP($q_sp, $q_ctsp)
	{
		...
		echo 	"<div class='lietke-sp-td' style='width: 18%'>".$re_sp['masp']."</div>";
		echo 	"<div class='lietke-sp-td' style='width: 31%'>".$re_sp['tensp']."</div>";
		echo 	"<div class='lietke-sp-td' style='width: 21%'>".$re_sp['tenncc']."</div>";
		echo 	"<div class='lietke-sp-td' style='width: 12%; text-align: center;' >".$re_sp['sl']."</div>";
		echo 	"<div class='lietke-sp-td' style='width: 6%; text-align: center;'><a  href='admin.php?quanly=sanpham&ac=suasp&masp=".$re_ctsp['masp']."'>Sửa</a></div>";
		echo 	"<div class='lietke-sp-td' style='width: 6%; text-align: center;'><a href='#'>Xóa</a></div>";
		echo 	"<div class='clear'></div>";
		echo "</div>";
